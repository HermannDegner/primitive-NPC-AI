    ...
    def step(self):
        ...
        if self.ticks_since_change >= self.change_interval:
            ...
            if self.condition == "sunny":
                # 晴れの日は晴れが続きやすいが、たまに雨になる
                if random.random() < 0.3:
                    self.condition = "rainy"
                    self.intensity = random.uniform(0.3, 0.8)
                else:
                    self.intensity = random.uniform(0.1, 1.0) # 晴れの強さが変わる
            else: # rainy
                # 雨の日は雨が続きやすいが、たまに晴れる
                if random.random() < 0.4:
                    ...
                    self.intensity = random.uniform(0.2, 1.0) # 雨の強さが変わる
...
class EnvForageBuff:
    def __init__(self, size=40, n_berry=18, n_hunt=10, n_water=5):
        ...
        self.weather = Weather() # Weatherオブジェクトを生成
...
class NPCPriority(NPCWithTerritory):
    ...
    def step(self, t, predator=None):
        ...
        if weather.condition == 'rainy' and weather.intensity > 0.8 and self.dist_to(self.territory.center) > 2:
            self.move_towards(self.territory.center)
            self.log.append({"t": t, "name": self.name, "action": "shelter_from_rain"})
            return

        # (L2, L3, L4 のロジックは前のコードから変更なし)
        # ... (コードが長いため省略)
        if self.thirst >= 95:
             if self.water > 0:
...
if __name__ == "__main__":
    final_npcs, df_logs, df_weather = run_sim(TICKS=800) # シミュレーション時間を延長して天候の影響を見る
    print("\n--- Simulation Finished ---")
