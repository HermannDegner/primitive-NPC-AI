...
class FuturePredictionEngine:
    ...
    def _initialize_action_costs(self) -> Dict[ActionType, Dict[str, float]]:
        ...
        return {
            ...
            ActionType.DRINK: {"fatigue": 8, "time": 1, "thirst_benefit": -35, "success_rate": 0.9},
            ActionType.REST: {"fatigue": -30, "time": 1, "min_recovery": -20, "safety_bonus": 15},
            ActionType.EXPLORE: {
                "fatigue": 12,
                "time": 1,
            ...
            ActionType.MOVE: {"fatigue": 2, "time": 1, "per_distance": True},
        }

    def _initialize_thresholds(self) -> Dict[str, Dict[str, float]]:
        """状態閾値の初期化"""
        return {
            "fatigue": {"safe": 40, "caution": 70, "danger": 100, "critical": 130},
            "hunger": {"satisfied": 20, "mild": 40, "serious": 60, "critical": 80},
            "thirst": {"satisfied": 15, "mild": 30, "serious": 50, "critical": 70},
        }
        ...
        for action in action_sequence:
            ...
            if action.action_type in self.action_costs:
                ...
                if "hunger_benefit" in costs:
                    ...
                    current_state["hunger"] += (
                        costs["hunger_benefit"] * success_rate * action.probability
                    )

                # 喉の渇き変化
                if "thirst_benefit" in costs:
                    success_rate = costs.get("success_rate", 1.0)
                    current_state["thirst"] += (
                        costs["thirst_benefit"] * success_rate * action.probability
                    )
        ...
        if self.npc.exploration_mode or self.npc.curiosity > 0.5:
            ...
            options.append(
                PredictedAction(
                    ...
                    benefit={
                        "knowledge": 0.8,
                        "discovery_chance": 0.3 + (0.2 if not is_night else -0.1),
                    },
    ...
    def evaluate_action_sequence(self, actions: List[PredictedAction]) -> float:
        ...
        if total_duration > 0:
            benefit_per_time = (
                sum(sum(action.benefit.values()) * action.probability for action in actions)
                / total_duration
            )
    ...
    def _assess_cooperation_potential(self) -> bool:
        ...
        nearby_npcs = [
            npc
            for npc in self.npc.roster.values()
            if (
                npc.alive
                and npc != self.npc
                and self.npc.distance_to(npc.pos()) <= 60
                and npc.fatigue < 120
            )
        ]
    ...
    def _has_safe_shelter(self) -> bool:
        """安全な避難所を持っているか"""
        return hasattr(self.npc, "knowledge_caves") and len(self.npc.knowledge_caves) > 0

    def _check_prerequisites(
        self, action: PredictedAction, current_sequence: List[PredictedAction]
    ) -> bool:
        """行動の前提条件チェック"""
