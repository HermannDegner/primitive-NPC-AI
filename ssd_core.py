...
class PhysicalStructureSystem:
    ...
    def process_structural_dynamics(self, external_stimuli):
        ...
        survival_mechanisms = self.foundation_layer.process_survival_mechanisms(
        ...
        # 補助情報を追加して返す（意味圧 p と整合流 j を上位に出す）
        try:
            p_norm = behavioral_decisions.get("integrated_pressure", {}).get("total", 0.0)
        except Exception:
            p_norm = 0.0

        try:
            j_norm = adaptive_outputs.get("final_decision", {}).get("pressure_resolved", 0.0)
        except Exception:
            j_norm = 0.0

        try:
            leap_prob = behavioral_decisions.get("leap_conditions", {}).get("probability", 0.0)
        except Exception:
            leap_prob = 0.0

        adaptive_outputs["p_norm"] = p_norm
        adaptive_outputs["j_norm"] = j_norm
        adaptive_outputs["meaning_pressure"] = p_norm
        adaptive_outputs["leap_probability"] = leap_prob
...
class PhysicalLayer:
    ...
    def __init__(self, npc):
        ...
        self.physical_constants = {
            ...
            "physical_limits": {"max_hunger": 200, "max_thirst": 180, "max_fatigue": 120},
        }
    ...
    def update_environmental_constraints(self, environmental_feedback):
        ...
        if "resource_scarcity" in environmental_feedback:
            ...
            self.physical_constants["energy_efficiency"] = max(
                0.5, self.physical_constants["energy_efficiency"]
            )
        ...
        if "climate_stress" in environmental_feedback:
            ...
            self.physical_constants["metabolic_rate"] = min(
                2.0, self.physical_constants["metabolic_rate"]
            )
...
class CoreLayer:
    ...
    def process_behavioral_decisions(self, survival_mechanisms):
        ...
        # 跳躍条件の評価
        decisions["leap_conditions"] = self._evaluate_leap_conditions(
            decisions["integrated_pressure"]
        )
    ...
    def _calculate_exploration_pressure(self):
        ...
        knowledge_saturation = (
            len(self.npc.knowledge_caves) * 10
    ...
    def _calculate_action_weights(self, mechanisms):
        ...
        if priorities["primary_need"] == "food":
            ...
            weights["drink"] *= 1.0 + priorities["urgency_level"]
            # 渇きが深刻な場合、探索も水源探しにシフト
            if priorities["urgency_level"] > 1.5:
                weights["exploration"] *= 1.5  # 水源探索を優先
...
class UpperLayer:
    ...
    def __init__(self, npc):
        self.npc = npc
        self.learning_rates = {"success_rate": 0.1, "failure_rate": 0.05, "adaptation_rate": 0.08}
    ...
    def _process_learning_updates(self, decisions):
        ...
        for action, weight in decisions["inertia_modulation"].items():
            # 使用頻度による慣性強化を中央 API で処理
            usage_boost = min(0.02, weight * 0.01)
            new_kappa = SSDCore.apply_kappa_update(self.npc, action, delta=usage_boost)
    ...
    def _adapt_long_term_strategy(self, decisions):
        ...
        strategy["exploration_strategy"] = self._adjust_exploration_strategy(
    ...
    def _calculate_exploration_success_rate(self):
        ...
        return (
            len(successful_explorations) / len(exploration_actions) if exploration_actions else 0.5
        )
    ...
    def _adapt_to_resource_availability(self):
        ...
        food_knowledge = len(self.npc.knowledge_berries) + len(
            getattr(self.npc, "knowledge_hunting", [])
        )
    ...
    def _make_final_decision(self, behavioral_decisions, adaptations):
        ...
        # 跳躍条件をチェック（理論に基づく指数型発火率）
        pressure = leap_conditions.get("pressure", 0.0)
        threshold = leap_conditions.get("threshold", THETA0)

        # compute poisson-like jump rate h = h0 * exp((E - Theta)/gamma)
        E_val = getattr(self.npc, "E", 0.0)
        h = H0 * math.exp((E_val - threshold) / GAMMA)
        jump_prob = 1.0 - math.exp(-h)  # dt assumed 1

        if random.random() < jump_prob:
            decision_type = "leap"
            # 未処理圧の部分解消（理論式に合わせた減衰）
            resolved = min(E_val, pressure * 0.2)
            self.npc.E = max(0.0, self.npc.E - resolved)
        else:
            decision_type = "normal"
            resolved = 0

        # 温度の算出：T = T0 + c1 * E - c2 * H(pi)
        # まず、行動重みを正規化して softmax 用のスコアを作る
        actions = list(modulated_weights.keys())
        scores = [modulated_weights[a] for a in actions]

        # compute preliminary temperature using E
        base_temp = getattr(self.npc, "T0", T0_NANO)
        temp = base_temp + C1_TEMP * E_val

        # softmax probabilities with temperature
        def softmax(vals, T):
            T = max(T, MIN_TEMPERATURE)
            exps = [math.exp(v / T) for v in vals]
            s = sum(exps)
            if s == 0:
                return [1.0 / len(vals)] * len(vals)
            return [e / s for e in exps]

        # compute policy entropy H(pi)
        probs = softmax(scores, temp)
        entropy = -sum([p * math.log(p + 1e-12) for p in probs])

        # adjust temperature by entropy term (higher entropy reduces T influence)
        temp = base_temp + C1_TEMP * E_val - C2_ENTROPY * entropy
...
class SSDCore:
    ...
    @staticmethod
    def update_E(npc, p_norm, j_norm):
        """中央の E 更新ルーチン（nano-SSD 準拠）"""

        # dE/dt = ALPHA_E * max(0, p - j) - BETA_E * E
        current_E = getattr(npc, "E", 0.0)
        dE = ALPHA_E * max(0.0, p_norm - j_norm) - BETA_E * current_E
        npc.E = max(0.0, min(E_MAX, current_E + dE))
        return npc.E
    ...
    @staticmethod
    def apply_kappa_update(npc, action_type, delta=0.0, min_kappa=0.05):
        """NPC の kappa を中央で安全に更新するユーティリティ。

        delta は正負どちらでも可能（増加または減少）。既存値が無ければ config の DEFAULT_KAPPA を使う。
        """
        current = npc.kappa.get(action_type, DEFAULT_KAPPA)
        new_kappa = max(min_kappa, min(1.0, current + delta))
        npc.kappa[action_type] = new_kappa
        return new_kappa
...
class ExplorationModeManager:
    ...
    def consider_exploration_leap(self, t, exploration_pressure):
        ...
        leap_probability = min(0.9, (exploration_pressure + self.npc.E * 0.3) / 2.0) * (
            0.5 + exploration_experience
        )
    ...
    def consider_mode_reversion(self, t, exploration_pressure):
        ...
        settlement_coherence = self._calculate_settlement_coherence(
            exploration_pressure, resource_stability
        )
        ...
        if (
            reversion_score > 0.6
            or (settlement_coherence >= coherence_threshold * 1.2)
            or (exploration_pressure < 0.2 and mode_duration > 8)
        ):

            self.npc.exploration_mode = False
            self.npc.exploration_intensity = 1.0
            # 減衰は中央 API で処理
            SSDCore.apply_kappa_update(
                self.npc,
                "exploration",
                delta=(
                    self.npc.kappa.get("exploration", 0.1) * 0.9
                    - self.npc.kappa.get("exploration", 0.1)
                ),
            )
    ...
    def _evaluate_resource_stability(self):
        ...
        food_stability = min(
            1.0, (len(self.npc.knowledge_berries) + len(self.npc.knowledge_hunting)) / 4.0
        )
